import { GetInTouchForm } from "@/components/widgets";
import Image from "next/image";
import { PiSealCheckFill } from "react-icons/pi";

export const FullPotentialSection = () => {
  return (
    <section className="bg-primary py-10">
      <div className="container mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-[133px]">
          <div>
            <div className="mb-6">
              <h2 className="sub-heading text-white">
                Unlock the full potential of your veterinary practice
              </h2>
            </div>
            <div className="body text-secondary mb-10 md:mb-16 text-white">
              Discover how Premier Vet Alliance's tailored solutions can enhance
              pet care and streamline your clinic’s operations. Book a
              personalized demo to see how our services can support the growth
              of your practice.
            </div>

            <div className="block lg:hidden mb-10">
              <GetInTouchForm />
            </div>

            <div className="relative md:pt-[260px] flex flex-col-reverse md:flex-row justify-end">
              <div className="md:absolute top-0 left-0 z-1 w-full -mt-10 md:mt-0 md:w-[411px] bg-[url('/images/double-circles.png')] bg-no-repeat rounded-2xl md:rounded-3xl shadow-paper px-4 md:px-8 py-6 md:py-9 bg-white">
                <div className="text-[32px] font-bold mb-6">
                  What can I expect?
                </div>
                <div className="flex flex-col gap-6">
                  <div className="flex items-start gap-3">
                    <PiSealCheckFill className="text-primary-light text-xl translate-y-0.5" />
                    <div className="flex-1">
                      Tailored health plans to enhance client retention.
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <PiSealCheckFill className="text-primary-light text-xl translate-y-0.5" />
                    <div className="flex-1">
                      Streamlined technology for easy communication and
                      scheduling.
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <PiSealCheckFill className="text-primary-light text-xl translate-y-0.5" />
                    <div className="flex-1">
                      10+ years of expertise and proven success.
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <PiSealCheckFill className="text-primary-light text-xl translate-y-0.5" />
                    <div className="flex-1">
                      Trusted by 3,000+ veterinary clinics worldwide.
                    </div>
                  </div>
                </div>
              </div>
              <Image
                className="w-full md:w-auto"
                src="/images/full-potential-cat.png"
                width={410}
                height={362}
                alt="cat"
              />
            </div>
          </div>
          <div className="hidden lg:block">
            <GetInTouchForm />
          </div>
        </div>
      </div>
    </section>
  );
};
